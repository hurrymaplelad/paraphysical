...
import { invalidStatementError, parsingError, unexpectedTokenError, } from "./errors.js";
import { LineLabelNumber, PositiveInt16 } from "./numbers.js";
import { ResidentPointSet, STATUS_NAMES, } from "./reserved.js";
import { tokenizeLine } from "./tokenizer.js";
const TIMED_STATEMENTS = new Set([
    "SAMPLE",
]);
export function parseFile(contents, context) {
...
export function parseLine(line, context) {
    ...
    const labelError = parsingError(`Line must start with ${LineLabelNumber.description}`, context);
    if (labelMatch == null) {
        throw labelError;
    }
...
export function parseStatement(tokens, context) {
...
// Statement Parsers
//
export function parseSAMPLE(tokens, context) {
    ...
    if (!PositiveInt16.isValid(secondsPerSample)) {
        throw parsingError(`SAMPLE seconds must be ${PositiveInt16.description}. Got ${secondsPerSample}`, context);
    }
    consumeExpected(tokens, ")", context);
    const sampledStatement = parseStatement(tokens, context);
    if (TIMED_STATEMENTS.has(sampledStatement.type)) {
        throw invalidStatementError({ outer: "SAMPLE", inner: sampledStatement.type }, context);
    }
    return {
        ...context,
        type: "SAMPLE",
...
export function parseConditional(tokens, context) {
...
function validateConditionalSubStatement(type, context) {
    if (FORBIDDEN_CONDITIONAL_SUB_STATEMENTS.has(type)) {
        throw invalidStatementError({ outer: "conditional", inner: type }, context);
    }
}
export function parseCallStatement(tokens, context) {
...
export function parseAssignment(tokens, context) {
...
export function parseGOTO(tokens, context) {
    consumeExpected(tokens, "GOTO", context);
    const { number: label } = consumeExpected(tokens, "number", context);
    if (!LineLabelNumber.isValid(label)) {
        throw parsingError(`Invalid GOTO: ${label}. Line number must be ${LineLabelNumber.description}`, context);
    }
    return {
        ...context,
        type: "GOTO",
...
export function parseGOSUB(tokens, context) {
    consumeExpected(tokens, "GOSUB", context);
    const { number: label } = consumeExpected(tokens, "number", context);
    if (!LineLabelNumber.isValid(label)) {
        throw parsingError(`Invalid GOSUB: ${label}. Line number must be ${LineLabelNumber.description}`, context);
    }
    const args = [];
    while (!tokens.isDone() && tokens.peek()?.type === "name") {
        if (args.length >= MAX_GOSUB_ARG_COUNT) {
            throw parsingError(`GOSUB supports at most ${MAX_GOSUB_ARG_COUNT} arguments`, context);
        }
...
export function parseExpression(tokens, context, initialLHS = null) {
...
export function parseCallExpression(tokens, context) {
...
export function parseLiteral(tokens, context) {
...
export function parseReference(tokens, context) {
...
export function parseReferenceIdentifier(identifier, _context) {
    ...
    if (ResidentPointSet.has(identifier)) {
        return {
            type: "residentPoint",
            name: identifier,
        };
