...
const clocks_js_1 = require("./clocks.js");
...
class Interpreter {
    constructor(options = null) {
        ...
        this.clock = options?.clock ?? new clocks_js_1.SystemClock();
    ...
    load(filename, contents) {
        const parsedFile = (0, parser_js_1.parseFile)(contents, { filename });
        const state = {
    ...
    dateTime() {
        return new datetime_js_1.DateTime(this.clock.getTimestamp(), this.timezone);
    }
    /**
    ...
    *runOnce(filename) {
        ...
        if (record == null) {
            throw (0, errors_js_1.fileNotLoaded)(filename);
        }
    ...
    evaluateStatement(statement) {
        switch (statement.type) {
            ...
            default:
                ...
                (0, errors_js_1.assertNever)(statement);
        }
    }
    ...
    evaluateAssignment(statement) {
        ...
        const dest = (0, parser_js_1.parseReferenceIdentifier)(lhs.identifier, statement);
        switch (dest.type) {
            case "local":
                return this.setLocal(dest.keyOrName, value, statement);
            case "point":
                if (reserved_js_1.SECONDS_COUNTER_REGEX.test(dest.name)) {
                    return this.setSecondsCounter(dest.name, value, statement);
                }
                return this.setPoint(dest.name, value);
        }
        throw (0, errors_js_1.runtimeError)(`cannot assign to ${lhs.identifier}`, statement);
    }
    ...
    evaluateGOSUB(statement) {
        ...
        if (fileState.gosubStack.length >= MAX_GOSUB_STACK_DEPTH) {
            throw (0, errors_js_1.runtimeError)(`Maximum of ${MAX_GOSUB_STACK_DEPTH} nested GOSUB calls exceeded`, statement);
        }
    ...
    evaluateRETURN(statement) {
        ...
        if (callsiteLabel == undefined) {
            throw (0, errors_js_1.runtimeError)("RETURN outside GOSUB", statement);
        }
    ...
    evaluateCallStatement(statement) {
        ...
        throw (0, errors_js_1.runtimeError)(`unrecognized function: ${functionName}`, statement);
    }
    evaluateLOCAL(args, context) {
        for (const arg of args) {
            if (arg.type !== "reference") {
                throw (0, errors_js_1.runtimeError)(`invalid LOCAL declaration: ${arg.type}`, context);
            }
            const name = arg.identifier;
            if (name.indexOf(LOCAL_DELIMITER) > -1) {
                throw (0, errors_js_1.runtimeError)(`invalid local declaration: ${name}`, context);
            }
            ...
            if (__classPrivateFieldGet(this, _Interpreter_instances, "m", _Interpreter_isLocalDeclared).call(this, key)) {
                throw (0, errors_js_1.runtimeError)(`attemped to re-declare LOCAL: ${name}`, context);
            }
    ...
    getLocal(nameOrKey, context) {
        ...
        if (value === undefined) {
            throw (0, errors_js_1.runtimeError)(`attempted to read undeclared LOCAL: ${nameOrKey}`, context);
        }
        return value;
    }
    setLocal(nameOrKey, value, context) {
        ...
        if (fileState == null || !__classPrivateFieldGet(this, _Interpreter_instances, "m", _Interpreter_isLocalDeclared).call(this, key)) {
            throw (0, errors_js_1.runtimeError)(`attempted to set undeclared LOCAL: ${key}`, context);
        }
    ...
    getPoint(name, context) {
        ...
        if (value == null) {
            throw (0, errors_js_1.runtimeError)(`point ${name} is not defined`, context);
        }
        return value;
    }
    setPoint(name, value) {
    ...
    evaluateReferenceExpression(expression, context) {
        const id = (0, parser_js_1.parseReferenceIdentifier)(expression.identifier, context);
        switch (id.type) {
            case "local":
                return this.getLocal(id.keyOrName, context);
            case "point":
                if (reserved_js_1.SECONDS_COUNTER_REGEX.test(id.name)) {
            ...
            case "status":
                return reserved_js_1.STATUS_NAMES[id.name];
        }
    }
    evaluateInfixBinaryOperation(expression, context) {
        ...
        switch (expression.operator) {
            ...
            default:
                (0, errors_js_1.assertNever)(expression.operator);
                throw (0, errors_js_1.runtimeError)(`operator ${expression.operator} not implemented`, context);
        }
    }
    evaluateCallExpression(expression, context) {
        ...
        throw (0, errors_js_1.runtimeError)(`unrecognized function: ${functionName}`, context);
    }
}
...
_Interpreter_files = new WeakMap(), _Interpreter_points = new WeakMap(), _Interpreter_currentFilename = new WeakMap(), _Interpreter_instances = new WeakSet(), _Interpreter_currentFileState = function _Interpreter_currentFileState(context) {
    ...
    if (record == null) {
        throw (0, errors_js_1.runtimeError)("No running file", context);
    }
    return record[1];
    ...
    switch (parts.length) {
        ...
        default:
            throw (0, errors_js_1.runtimeError)(`invalid local: ${nameOrKey}`, context);
    }
